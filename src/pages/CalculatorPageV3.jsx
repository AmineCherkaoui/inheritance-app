...
import { cn, deserializeState, serializeState } from "../utils";
...
import {
  HEIR_CATEGORIES,
  isHeirBlocked,
} from "../components/calculatorV3/heirConstants";
...
const ALL_MINI_STEPS = [
  ...
  {
    key: "father_uncles_cousins",
    parentStep: 2,
    label: "أعمام الأب وأبناء عمومتهم",
  },
];
...
const isStepBlocked = (stepKey, currentHeirs, gender) => {
  ...
  if (stepKey === "children") {
    return (
      isHeirBlocked("SON", currentHeirs) &&
      isHeirBlocked("DAUGHTER", currentHeirs)
    );
  }
  if (stepKey === "parents") {
    return (
      isHeirBlocked("FATHER", currentHeirs) &&
      isHeirBlocked("MOTHER", currentHeirs)
    );
  }

  const catKey = stepKey;
  if (HEIR_CATEGORIES[catKey]) {
    return HEIR_CATEGORIES[catKey].list.every((heir) =>
      isHeirBlocked(heir.key, currentHeirs),
    );
  }
  return false;
};
...
export default function CalculatorPageV3() {
  ...
  useEffect(() => {
    ...
    if (sharedStateStr) {
      ...
      if (state) {
        if (state.deceasedName !== undefined)
          setDeceasedName(state.deceasedName);
        if (state.deceasedGender !== undefined)
          setDeceasedGender(state.deceasedGender);
        ...
        if (state.heirsApprovedExcess !== undefined)
          setHeirsApprovedExcess(state.heirsApprovedExcess);
        ...
        if (
          heirsList.length > 0 ||
          (state.mandatoryBequests && state.mandatoryBequests.length > 0)
        ) {
          const caseData = {
            id: Date.now(),
            name:
              state.deceasedName ||
              (state.deceasedGender === "male" ? "المتوفى" : "المتوفاة"),
  ...
  const activeMiniSteps = ALL_MINI_STEPS.filter((step) => {
    if (
      step.key === "deceased_info" ||
      step.key === "estate_debts" ||
      step.key === "wills"
    ) {
      return true;
    }
  ...
  const currentMiniStep =
    activeMiniSteps[activeMiniStepIndex] || activeMiniSteps[0];
  const currentParentStep = isViewingResults
    ? 3
    : (currentMiniStep?.parentStep ?? 0);

  const cleanHeirs = (
    updatedHeirs,
    gender = deceasedGender,
    mandatory = hasMandatoryBequest,
  ) => {
  ...
  const handleCalculate = () => {
    ...
    if (
      heirsList.length === 0 &&
      (!hasMandatoryBequest || mandatoryBequests.length === 0)
    ) {
      setErrorMessage(
        "الرجاء إضافة وارث واحد على الأقل للمسألة في الخطوات السابقة.",
      );
      return;
    }
    ...
    const caseData = {
      id: Date.now(),
      name:
        deceasedName || (deceasedGender === "male" ? "المتوفى" : "المتوفاة"),
      gender: deceasedGender,
  ...
  const handleNext = () => {
    ...
    if (currentMiniStep.key === "estate_debts") {
      if (!totalEstate || parseFloat(totalEstate) <= 0) {
        setErrors({
          totalEstate: "يرجى إدخال قيمة التركة الإجمالية أكبر من الصفر.",
        });
        return;
      }
    }
    ...
    if (currentMiniStep.key === "wills") {
      ...
      if (
        hasMandatoryBequest &&
        (mandatoryBequests.length === 0 || hasZeroCount)
      ) {
  ...
  const handleSidebarStepClick = (parentIndex) => {
    ...
    setIsViewingResults(false);
    const targetMiniStepIdx = activeMiniSteps.findIndex(
      (s) => s.parentStep === parentIndex,
    );
  ...
  const canNavigateToStep = (parentIndex) => {
    ...
    if (parentIndex === 2)
      return (
        Boolean(deceasedGender) && totalEstate && parseFloat(totalEstate) > 0
      );
    if (parentIndex === 3) {
      const hasHeirs =
        Object.values(heirs).some((count) => count > 0) ||
        (hasMandatoryBequest && mandatoryBequests.length > 0);
      return (
        Boolean(deceasedGender) &&
        Boolean(totalEstate) &&
  ...
  return (
    ...
    >
      <AppBackground />
      <div className="lg:hidden bg-primary-950 py-2 px-4 sticky top-0 backdrop-blur-sm border-b border-primary-950 z-50 flex items-center justify-between">
        <MobileNav
          ...
          <div className="flex-1 flex flex-col justify-between gap-4 h-full w-full  ">
                ...
                <AnimatePresence mode="wait">
                  {isViewingResults ? (
                    <StepResults
                      ...
                      onBackToEdit={() => {
                        setIsViewingResults(false);
                        setActiveMiniStepIndex(
                          Math.max(0, activeMiniSteps.length - 1),
                        );
                      }}
                      onReset={resetAll}
            ...
            <div className="flex items-center justify-center gap-12">
              ...
              <button
                ...
                className={cn(
                  "px-6 py-3 rounded-md hover:bg-primary-950/10 ring-1 ring-primary-950 bg-transparent text-primary-950 font-bold text-xs sm:text-sm transition-all flex items-center gap-1.5",
            ...
            <div className="mt-8 w-full bg-secondary-100  text-secondary-400/60 py-2 px-4 rounded-sm text-center text-xs flex items-center justify-center gap-2">
              ...
              <span>
                جميع الحسابات تتم وفق أحكام الشريعة الإسلامية والمذهب المالكي
                المعتمد في المملكة المغربية الشريفة
              </span>
            </div>
          </div>
        </section>
