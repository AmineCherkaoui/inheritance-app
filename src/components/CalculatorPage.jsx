...
export function CalculatorPage() {
    ...
    const cleanHeirs = (updatedHeirs, currentGender = deceasedGender, mandatory = hasMandatoryBequest) => {
        const clean = { ...updatedHeirs };
        for (const key of Object.keys(clean)) {
            if (isHeirBlocked(key, clean)) {
                delete clean[key];
            }
            if (currentGender === 'male' && key === 'HUSBAND') {
                delete clean[key];
            }
            if (currentGender === 'female' && key === 'WIFE') {
                delete clean[key];
            }
    ...
    const updateHeir = (key, val) => {
        ...
        setHeirs(prev => {
            ...
            const cleaned = cleanHeirs(updated);
            const activeCount = Object.values(cleaned).filter(c => c > 0).length;
            if (activeCount > 0) {
                setErrorMessage('');
            }
            return cleaned;
    ...
    const handleCalculate = () => {
        ...
        navigate(`/results?s=${sharedStateStr}`, { state: { result: output, from: '/' } });
    };
    ...
    return (
        <div className="min-h-screen bg-background pb-16 pt-8 px-4 sm:px-6 lg:px-8" dir="rtl">
            ...
            {/* Mandatory Bequest Card (Separate from Wills) */}
